# 🚶 BrightWalk

**AI-powered navigation assistant for visually impaired users.**

BrightWalk combines computer vision, location intelligence, and voice interaction to help users explore their surroundings independently — hands-free.

> *"See the world. Hear the way. Walk independently."*

<p align="center">
  <a href="https://drive.google.com/file/d/1Nt0TL_iIS2I8RudvxY6JuJZow8BulwS-/view?usp=share_link">
    <img src="https://img.shields.io/badge/▶️_Watch_Demo-4285F4?style=for-the-badge&logo=googledrive&logoColor=white" alt="Watch Demo Video">
  </a>
</p>

---

## 🔧 Prototype & Demo

<p align="center">
  <img src="https://github.com/user-attachments/assets/79b4814a-4be4-4ae0-a644-2a8996077fc3" width="260">
  <img src="https://github.com/user-attachments/assets/b7bb6cd6-75cf-4ba1-9c80-667261070db7" width="200">
  <img src="https://github.com/user-attachments/assets/89fee998-fdc2-41fb-9317-a9d1a7bb8f90" width="200">
</p>

<p align="center"><i>ESP32-CAM prototype build · Live demo & AI scene description</i></p>

---

## ✨ Features

- 🗺️ **Explore Nearby** — attractions, restaurants & museums with ratings and distance
- 👁️ **Visual Understanding** — camera-based AI scene description
- 🔊 **Voice Assistance** — spoken instructions and audio descriptions
- 🗣️ **Voice Control** — full hands-free interaction via accessibility services
- 🌐 **Multi-Language Support** — voice output in multiple languages
- ♿ **Accessibility-First UI** — high contrast, large controls, minimal clutter

---

## 📱 Screenshots

<p align="center">
  <img src="https://github.com/user-attachments/assets/b41afe2e-f6c8-4185-acfb-f5813be35e0c" width="200">
  <img src="https://github.com/user-attachments/assets/3faead3a-5e9c-4c89-9a75-95bfadd3e51e" width="200">
  <img src="https://github.com/user-attachments/assets/9de5f6ca-396f-4e27-bd9c-653d8e432a18" width="200">
  <img src="https://github.com/user-attachments/assets/9dbf9ec1-0f7b-4d85-bbbb-15422b09b588" width="200">
</p>

<p align="center"><i>Accessibility setup · Language selection · Explore Nearby · Voice interaction</i></p>

---

## 🛠️ Tech Stack

Kotlin · CameraX · Google ML Kit · Google Maps SDK · Location APIs · Text-to-Speech · Android Accessibility APIs · Jetpack · Material UI

---

## 🚀 Getting Started

```bash
git clone https://github.com/codekatana07/Bright-Walk.git
cd Bright-Walk
